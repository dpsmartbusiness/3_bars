...
def get_biggest_bar(bars):
    ...
    for bar in bars["features"]:
        b_bar[bar["properties"]["Attributes"]["SeatsCount"]] = bar["properties"]["Attributes"]["Name"]
...
def get_smallest_bar(bars):
    s_bar = dict()
    for bar in bars["features"]:
        s_bar[bar["properties"]["Attributes"]["SeatsCount"]] = bar["properties"]["Attributes"]["Name"]
...
def get_closest_bar(bars, longitude, latitude):
    bar_coord = dict()
    for bar in bars["features"]:
        bar_coord[bar["properties"]["Attributes"]["Name"]] = bar['geometry']['coordinates']
    c_bar = dict()
...
if __name__ == '__main__':
    ...
    answ = int(input("Задайте требуемый критерий для поиска цифрой от 1го до 3х: \n"))
